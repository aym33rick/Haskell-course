...
import Test.QuickCheck -- :set -package QuickCheck
...
-- pass the (potentialy) infinite list)
...
rList f random = 
    ...
    in if (isNil<20) 
       then ([],random1)
       else let (head,random2) = f       random1 -- generate one element
                (tail,random3) = rList f random2
            in (head:tail,random3)
...
-- int m1(int x);
-- int m1(boolean x);
-- void m1(boolean x); -- interdit en java, mais autorise en haskell
...
-- returnM :: a -> M a  -- element neutre de returnM
