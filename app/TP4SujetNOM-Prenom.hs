...
nbOccurrences [] = []
nbOccurrences (head : tail) = (head,length(filter (\e -> e == head) tail) + 1 ) : nbOccurrences (filter (\e -> e /= head) tail)
...
insere leaf (head: tail) | poids leaf <= poids head = leaf : head : tail
insere leaf (head: tail) = head : insere leaf tail
insere leaf [] = [leaf]
...
agrege (first:[])= first
agrege (first:seconde:tail)= agrege ( insere (Node first seconde) tail)
...
codeOne' :: Huff Char -> Char -> [Bits]
codeOne' (Leaf leaf) char | leaf == char = [[]]
codeOne' (Leaf leaf) char = []
codeOne' (Node l r) char = map (L :) (codeOne' l char) ++ map (R :) (codeOne' r char)
...
decode _ _ [] = []
decode root (Node (Leaf l) _) (L:bits) = l : decode root root bits
decode root (Node _ (Leaf r)) (R:bits) = r : decode root root bits
decode root (Node _ r) (R:bits) = decode root r bits
